...
- Specific LLM provider/model — swappable implementation detail, doesn't affect specs or task breakdown. Implemented with Google Gemini (free tier) rather than a paid provider, since this is a single-candidate hobby project and free-tier usage comfortably covers it; the LLM call boundary (`callStructured`) is provider-agnostic so this can change without touching extraction/scoring/matching/suggestion logic. Pinned to a specific model (`gemini-3.5-flash-lite` as of this writing) rather than the `gemini-flash-latest` alias — the alias seemed like the right way to dodge deprecation churn, but backfired in practice: it silently drifted to a newer model within the same day, and every user of that alias shares one 20-req/day free-tier quota bucket tied to whatever it currently points to, which real testing exhausted. A pinned model has its own separate quota and needs manual updating if it's deprecated, which is the better tradeoff of the two.
