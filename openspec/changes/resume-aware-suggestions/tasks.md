...
**Aside found during verification:** a different JD/resume pair (near-synonym skills "A/B Testing" / "Experimentation Design") reproducibly crashed `createAnalysis` with a duplicate-key DB error, unrelated to this change - the taxonomy matcher can resolve two distinct extracted names to one canonical skill, and the JD-requirements/resume-evidence bulk inserts had no dedup for that case. Root-caused and fixed separately (see `backend/src/services/intake.ts`, commit `ef27832`), with regression tests and a confirmed live re-test (3/3 stable) before returning to close out this change.
